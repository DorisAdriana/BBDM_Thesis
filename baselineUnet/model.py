...
#import functions
from functions import image_proc, conv_block, encoder_block, decoder_block, build_2DUNet_model_v1, plot_learning_curve
...
# Read and process the images (crop, resize, & normalize)
# Source images
dataset_path = 'baselineUnet/dataset/' 
dir_list_sc = sorted(glob.glob(dataset_path + 'input/*nii')) 
print(dir_list_sc)
...
# Target images
dataset_path = 'baselineUnet/dataset/' 
dir_list_tg = sorted(glob.glob(dataset_path + 'target/*nii'))
print(dir_list_tg)
...
# Callbacks
checkpoint_filepath = 'saved_model/MR_synth_best_model.epoch{epoch:02d}-loss{val_loss:.2f}.keras'
callbacks = [
...
# Source images
dataset_path = 'baselineUnet/dataset/' # MAKE TRAIN AND TST DIRS #'E:/Datasets/'
dir_list_sc = sorted(glob.glob(dataset_path + 'input/*nii')) # sorted(glob.glob(dataset_path + '/MR_images_T2/test-pats/*nii.gz'))
img_sc = image_proc(dir_list_sc) 
img_sc = np.expand_dims(img_sc, -1)

# Target images
dataset_path = 'baselineUnet/dataset/' # MAKE TRAIN AND TST DIRS #'E:/Datasets/'
dir_list_tg = sorted(glob.glob(dataset_path + 'target/*nii')) # sorted(glob.glob(dataset_path + '/MR_images_T2/test-pats/*nii.gz'))
img_tg = image_proc(dir_list_sc) 
img_tg = np.expand_dims(img_sc, -1)
